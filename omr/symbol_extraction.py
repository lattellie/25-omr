...
from omr.inference import predict
from omr.utils import get_global_unit_size, slope_to_degree, get_unit_size, find_closest_staffs
from omr.logger import get_logger
from omr.general_filtering_rules import filter_out_of_range_bbox, filter_out_small_area
...
def symbol_extract(min_barline_h_unit_ratio: float = 3.75) -> Tuple[List[Barline], List[Clef], List[Sfn], List[Rest]]:
    # Fetch paramters
    symbols = layers.get_layer('symbols_pred')
    stems_rests = layers.get_layer('stems_rests_pred')
