    ...
    for barThisTrack in barEachTrack:
        ...
        if len(barThisTrack) > 1:
            while prevIdx < len(barThisTrack)-1:
                if barThisTrack[prevIdx+1] - barThisTrack[prevIdx] > 5:
...
if __name__ == '__main__':
    ...
    for trkRange in pageMetadata.getTrackRange():
        ...
        barEachTrack += barPos
    barList,allRanges, numBarsEachLine = constructBar(noteGroupMap, noteGroupVerticallyMerged, restMap ,restList, sfnClefMap, sfnClefList, beamMapImg, staffList, pageMetadata, barEachTrack)
