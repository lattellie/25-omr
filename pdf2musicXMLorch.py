...
if __name__ == '__main__':
    ...
    staffList: List[Staff]
    symNum = 6
    sheetName = f'Bee_{symNum}_challenge'
    prevBarTS = [12,8]
    lenString = 3
    runWholeModel = True
    fullBarList = []
    ...
    dataSeperate = dataSepList[f'bee{symNum}']
    for number in range(39,40): #(51,53):
        ...
        rest_colors = [(255,0,0),(0,0,255),(255,255,0),(0,120,255),(40,255,40), (245, 220, 255),(230,130,175),(165,170, 70)]
        knn_colors = [(255,0,0),(0,0,255),(255,255,0),(0,120,255),(40,255,40), (245, 220, 255),(230,130,175),(165,170, 70)]
        imgrgb = image.copy()
        beamWidth = 9
        for oneTrack in barList:
            for oneBar in oneTrack:
                for elem in oneBar.elementList:
                    if type(elem) == NoteGroup:
                        for stem in elem.noteStemList:
                            x0,y0,x1,y1 = stem.noteBox
                            imgrgb = cv2.rectangle(imgrgb, (stem.getX()-beamWidth//3, stem.getTopCoord()[1]), (stem.getX()+beamWidth//3, stem.getBottomCoord()[1]), knn_colors[stem.rhythm], 1, cv2.LINE_AA)
                            imgrgb = cv2.rectangle(imgrgb, (x0,y0),(x1,y1), knn_colors[stem.rhythm], 1, cv2.LINE_AA)
                    elif type(elem) == Rest:
                        x0,y0,x1,y1 = elem.boundingBox
                        imgrgb = cv2.rectangle(imgrgb, (x0,y0),(x1,y1),rest_colors[elem.rhythm], 2, cv2.LINE_AA)
        cv2.imwrite('test.jpg',imgrgb)
